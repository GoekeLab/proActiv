...
filesSTAR <- list.files(system.file('extdata/testdata/star', package = 'proActiv'), 
                        full.names = TRUE, pattern = 'sample1|sample2')
...
test_that('calculateJunctionReadCounts returns expected output',{
  ...
  expect_equal(as.numeric(calculateJunctionReadCounts(promoterCoordinates, intronRanges, filesTophat[1], 'tophat')), 
               promoterCounts$sample1)
  ...
  expect_equal(as.numeric(calculateJunctionReadCounts(promoterCoordinates, intronRanges, filesSTAR[1], 'star')), 
               promoterCounts$sample1)
...
test_that('calculatePromoterReadCounts returns expected output', {
  ...
  expect_equivalent(calculatePromoterReadCounts(promoterAnnotation, filesTophat, fileLabels, 'tophat'), 
                    promoterCounts)
  ...
  expect_equivalent(calculatePromoterReadCounts(promoterAnnotation, filesSTAR , fileLabels, 'star'), 
                    promoterCounts)

  ### 3) BAM input ###
  expect_type(
    suppressWarnings(calculatePromoterReadCounts(promoterAnnotation.gencode.v34, bamfiles, fileLabels, 'bam', 'hg38')), 
    'list'
  )
  expect_equal(
    suppressWarnings(
      dim(calculatePromoterReadCounts(promoterAnnotation.gencode.v34, bamfiles, fileLabels, 'bam', 'hg38'))), 
...
test_that('normalizedReadCounts returns expected output', {
  ...
  normalizedPromoterCounts <- readRDS(system.file('extdata/testdata/tophat2/normalizedPromoterCounts.rds', 
                                                  package = 'proActiv'))
